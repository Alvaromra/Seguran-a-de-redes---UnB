# Constants for AES
BLOCK_SIZE = 16  # AES block size in bytes

# AES S-Box and Inverse S-Box
S_BOX = [
    
]

INV_S_BOX = [S_BOX.index(i) for i in range(256)]  # Calculate inverse S-Box

# Helper functions for AES
def sub_bytes(state, inverse=False):
    box = INV_S_BOX if inverse else S_BOX
    for i in range(4):
        for j in range(4):
            state[i][j] = box[state[i][j]]
    return state

def shift_rows(state, inverse=False):
    for i in range(1, 4):
        if inverse:
            state[i] = state[i][-i:] + state[i][:-i]
        else:
            state[i] = state[i][i:] + state[i][:i]
    return state

def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]
    return state

def print_state(state, label):
    print(f"{label} State:")
    for row in state:
        print(" ".join(f"{x:02x}" for x in row))
    print()

def aes_encrypt(block, key, num_rounds):
    state = [row[:] for row in block]
    round_keys = [key] * (num_rounds + 1)

    print_state(state, "Initial")

    state = add_round_key(state, round_keys[0])

    for i in range(1, num_rounds):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = add_round_key(state, round_keys[i])

    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[num_rounds])

    print_state(state, "Final")
    return state

def aes_decrypt(block, key, num_rounds):
    state = [row[:] for row in block]
    round_keys = [key] * (num_rounds + 1)

    print_state(state, "Initial")

    state = add_round_key(state, round_keys[num_rounds])

    for i in range(num_rounds - 1, 0, -1):
        state = shift_rows(state, inverse=True)
        state = sub_bytes(state, inverse=True)
        state = add_round_key(state, round_keys[i])

    state = shift_rows(state, inverse=True)
    state = sub_bytes(state, inverse=True)
    state = add_round_key(state, round_keys[0])

    print_state(state, "Final")
    return state

if __name__ == "__main__":
    print("AES Encryption and Decryption Program")
    num_rounds = int(input("Enter the number of AES rounds (e.g., 10, 12, 14): "))

    test_block = [
        [0x32, 0x88, 0x31, 0xe0],
        [0x43, 0x5a, 0x31, 0x37],
        [0xf6, 0x30, 0x98, 0x07],
        [0xa8, 0x8d, 0xa2, 0x34]
    ]
    test_key = [
        [0x2b, 0x7e, 0x15, 0x16],
        [0x28, 0xae, 0xd2, 0xa6],
        [0xab, 0xf7, 0xcf, 0x4f],
        [0x30, 0xc0, 0x39, 0x07]
    ]

    print("Encrypting...")
    encrypted_block = aes_encrypt(test_block, test_key, num_rounds)
    print("Encrypted Block:", encrypted_block)

    print("Decrypting...")
    decrypted_block = aes_decrypt(encrypted_block, test_key, num_rounds)
    print("Decrypted Block:", decrypted_block)
